...
function registrar_leitura(qtd) {

    console.log('Iniciando inclusão de novo registro...');
    console.log(`Quantidade Pessoas: ${qtd}`);

    if (qtd > 0) {
        banco.conectar().then(() => {

            return banco.sql.query(`
            INSERT into tbMovimento (fkSensor, dataMovimento, localizacao, leitura)
            values (1, CONVERT(Datetime, '${agora()}', 120), ${(Math.random()*200).toFixed(2)}, ${qtd});
            
            delete from tbMovimento where codMovimento not in 
            (select top ${registros_mantidos_tabela_leitura} codMovimento from tbMovimento order by codMovimento desc);`);
        ...
        console.log('Nenhuma movimentação detectada. \n')
    }
...
if (gerar_dados_aleatorios) {
    ...
    setInterval(function () {
        ...
        registrar_leitura(parseInt(Math.random() * 16));
