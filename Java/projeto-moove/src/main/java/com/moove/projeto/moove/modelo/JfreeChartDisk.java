...
public class JfreeChartDisk extends JFrame {
    ConexaoBanco conexaoLocal = new ConexaoBanco();
    ConexaoBancoAzure conexaoAzure = new ConexaoBancoAzure();
    public JfreeChartDisk( String applicationTitle , String chartTitle) {
   ...
   private DefaultCategoryDataset createDataset() {
      ...
      ("select disk_load, id from ohwLeituraWind order by id limit 10", new BeanPropertyRowMapper(DadosAPI.class));
       for (DadosAPI listdado : listdados) {
            listaId.add(listdado.getId());
            listaValores.add(Double.parseDouble(listdado.getDisk_load()));
      }
