...
public class JfreeChart extends JFrame {
   ...
   private DefaultCategoryDataset createDataset() {
      ...
      ("select cpu_load, id from ohwLeituraWind order by id limit 10", new BeanPropertyRowMapper(DadosAPI.class));
       for (DadosAPI listdado : listdados) {
            listaId.add(listdado.getId());
            listaValores.add(Double.parseDouble(listdado.getCpu_load()));
      }
